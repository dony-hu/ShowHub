...
const toolGroups = [
  {
    title: '可信事实',
    desc: '地址、AOI、道路、楼栋、企业、轨迹与行业专题数据，为 AI 应用提供可验证的真实世界输入。',
  },
  {
    title: '空间推理',
    desc: '距离、可达、覆盖、时序变化、风险叠加与实体关系，让系统不只回答问题，还能形成判断依据。',
  },
  {
    title: '工具执行',
...
const scenarios = [
  ['地址治理', '识别脏地址', '标准化解析', '匹配空间实体', '生成修正建议'],
  ['物流调度', '理解订单压力', '分析网点覆盖', '评估路径时效', '推荐调度策略'],
  ['警务研判', '接收警情', '定位现场', '叠加风险实体', '生成处置方案'],
  ['城市治理', '接入多源数据', '空间落位', '实体归并', '生成治理专题'],
]
    ...
    <div className="agent-page">
      <section className="agent-hero">
        <div className="agent-hero-inner">
          <div className="agent-hero-copy">
            <p className="agent-eyebrow">AI-ready Spatial Intelligence</p>
            <h1>让 AI 理解真实世界空间</h1>
            <p className="agent-hero-desc">
              丰图基于真实物流场景、可信时空数据与空间智能平台，为企业应用、城市治理和行业决策系统提供
              可调用、可推理、可验证的真实世界空间基础设施，并适配 AI 自动化时代的执行需求。
            </p>
          ...
          <div className="agent-console" aria-label="AI task flow">
            ...
            <div className="console-line">工具调用：地址解析 / AOI 匹配 / 路径规划 / 覆盖分析</div>
      ...
      <section className="agent-section agent-why">
        <div className="agent-section-head">
          ...
          <p>
            当 AI 深入配送、调度、研判、治理、选址和应急任务，它必须知道地址是否真实、实体在哪里、道路是否可达、
            区域如何变化、行动是否合规。这正是空间智能的价值边界。
          </p>
        </div>
      ...
      <section className="agent-section agent-architecture">
        <div className="agent-section-head">
          ...
          <h2>丰图 AI 空间智能架构</h2>
          <p>从真实世界数据到行业智能应用，丰图把空间能力沉淀成可持续调用的基础设施。</p>
        </div>
        <div className="agent-layers">
          ...
          <div>
            <span>04</span>
            <strong>行业智能应用层</strong>
            <p>地址治理、物流调度、警务研判、政数治理、环卫调度与行业智能体应用。</p>
          </div>
        </div>
      </section>
      ...
      <section className="agent-cta">
        <h2>从可信数据，到可行动的空间智能</h2>
        <p>丰图不只是让地图更智能，而是让 AI 应用真正理解空间、理解业务、理解真实世界。</p>
